 ...
 * 							Back-End			Front-End
 * 			사용처			관리페이지				모바일 => 동적
 *                          | 공공기업 사이트		| 페이징 / 검색 / 예약 / 결제 / 채팅 / AI / GPT                          					
 * 			-----------------------------------------------------
 * 			=> 기본 : ThymeLeaf / JSP
 * 			=> 동적 : React / Vue
 * 
 *  		1. DOM (ML => 저장 될때 트리형태로 저장)
 ...
 *              
 *          4. 디렉티브
 *              = v-model : 입력시에 data()에 있는 변수의 값을 설정
 *                 | 양방향
 *                 | React에 없다 => onchange
 *              = v-bind  : 속성에 값을 설정 => :
 *                 <img v-bind:src="vo.poster"> ==> <img :src="vo.poster">
 *              = v-on    : 이벤트 처리
 *                 <button v-on:click="함수"> ==> <button @click="함수">
 *                              mouseover
 *              = v-for   : 반복분 => for-each만 지원
